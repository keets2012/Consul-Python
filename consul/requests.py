import urllib2
from consul import __version__ as client_version


class ConsulHTTPException(Exception):
    pass
...
class Request(urllib2.Request):
    ...
    def __init__(self, url, method="GET", data=None, headers=None,
        ...
        self._opener.addheaders = [
            ('User-agent', 'python-Consul v%s' % client_version)
        ]
    ...
    def create(cls, method, url, data=None, headers=None):
        ...
        return Response(response.getcode(), response.read(), url, method), res
...
class Response(object):
    ...
    def raise_for_status(self):
        if not (200 <= self.status_code < 300):
            ...
            raise ConsulHTTPException(u"HTTP %s: %s" % (self.status_code, self.content))
